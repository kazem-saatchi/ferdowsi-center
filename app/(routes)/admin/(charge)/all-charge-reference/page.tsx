...
export default function ChargeReferenceListPage() {
  const { data, isLoading, isError, error, refetch } = useFindAllChargesReference()

  const { allChargesReference, setAllChargesReference, allAnnualChargeReference, setAllAnnualChargesReference } =
    useStore(
      useShallow((state) => ({
  ...
  const sortedChargeReferences = [...(allChargesReference || []), ...(allAnnualChargeReference || [])]
  ...
  if (isError) return <ErrorComponent error={error} message={labels.errorOccurred} retry={refetch} />

  return (
    <Card>
      <CardHeader>
        <div className="flex flex-row items-center justify-between">
          <CardTitle>{sortConfig.proprietor ? labels.proprietorChargeList : labels.monthlyChargeList}</CardTitle>
          <Button
            variant="outline"
            onClick={() => {
              setSortConfig((prev) => {
          ...
          >
            {sortConfig.proprietor ? labels.viewMonthlyList : labels.viewProprietorList}
          </Button>
        </div>
      </CardHeader>
      <CardContent>
          ...
          <TableBody>
            {sortedChargeReferences.map((reference) => (
              <TableRow key={reference.id}>
                ...
                <TableCell className="text-center">{reference.area.toFixed(2)}</TableCell>
                <TableCell className="text-center text-2xl">{formatNumber(reference.constantAmount)}</TableCell>
                <TableCell className="text-center text-2xl">{formatNumber(reference.metricAmount)}</TableCell>
                <TableCell className="text-center text-2xl">{formatNumber(reference.totalAmount)}</TableCell>
